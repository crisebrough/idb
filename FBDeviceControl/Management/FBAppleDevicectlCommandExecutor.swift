...
public class FBAppleDevicectlCommandExecutor: NSObject {
  ...
  @objc public func taskBuilder(arguments: [String]) -> IDBProcessBuilder<NSNull, NSString, NSString> {
    let derivedArgs = ["devicectl"] + arguments
    return IDBProcessBuilder<NSNull, NSString, NSString>.withLaunchPath("/usr/bin/xcrun", arguments: derivedArgs)
