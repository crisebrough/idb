...
+ (FBFuture<IDBProcess *> *)internalOperationWithArguments:(NSArray<NSString *> *)arguments queue:(dispatch_queue_t)queue logger:(nullable id<FBControlCoreLogger>)logger
{
...
+ (FBFuture<IDBProcess *> *)exportFrom:(NSString *)path to:(NSString *)destination forId:(NSString *)bundleObjectId withType:(NSString *)exportType queue:(dispatch_queue_t)queue logger:(nullable id<FBControlCoreLogger>)logger
{
...
+ (NSDictionary<NSString *, NSDictionary<NSString *, id> *> *)getJSONFromTask:(IDBProcess *)task
{
...
+ (FBFuture<IDBProcess *> *)exportFileFrom:(NSString *)path to:(NSString *)destination forId:(NSString *)bundleObjectId queue:(dispatch_queue_t)queue logger:(nullable id<FBControlCoreLogger>)logger
{
...
+ (FBFuture<IDBProcess *> *)exportJPEGFrom:(NSString *)path to:(NSString *)destination forId:(NSString *)bundleObjectId type:(NSString *)encodeType queue:(dispatch_queue_t)queue logger:(nullable id<FBControlCoreLogger>)logger
{
  return [[FBXCTestResultToolOperation
   exportFileFrom:path to:destination forId:bundleObjectId queue:queue logger:logger]
   onQueue:queue fmap:^ FBFuture * (IDBProcess *task) {
...
+ (FBFuture<IDBProcess *> *)exportDirectoryFrom:(NSString *)path to:(NSString *)destination forId:(NSString *)bundleObjectId queue:(dispatch_queue_t)queue logger:(nullable id<FBControlCoreLogger>)logger
{
